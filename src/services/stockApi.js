/**
 * Interfaz de comunicación asíncrona con el backend de Yahoo Finance.
 * Emplea peticiones GET y depende de un proxy público para resolver
 * las políticas de CORS (Cross-Origin Resource Sharing) requeridas por los navegadores modernos.
 */
 ...
 * Retorna las métricas del último tick del mercado o los datos predefinidos de respaldo
  ...
  try {
    const promises = SYMBOLS.map(async (symbol) => {
      try {
        ...
        if (!meta || !quote || meta.regularMarketPrice === undefined) {
          throw new Error('Datos incompletos o precio no disponible')
        }
        ...
        if (previousClose && previousClose > 0) {
          variation = (((currentPrice - previousClose) / previousClose) * 100).toFixed(2)
        }
